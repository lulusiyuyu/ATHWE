...
def run_recbole(model, dataset, config_file_list, saved=True):
    ...
    # Log additional stability configurations
    logger.info(set_color("Gradient Clipping", "blue") + f": {config['grad_clip_norm']}")
    logger.info(set_color("Learning Rate", "blue") + f": {config['learning_rate']}")
    logger.info(set_color("Weight Decay", "blue") + f": {config['weight_decay']}")
